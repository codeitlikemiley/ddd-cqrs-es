...
echo "auth-stack live OAuth preflight: checking $BASE_URL"
...
for provider in $OAUTH_PROVIDERS; do
  ...
  echo "auth-stack live OAuth preflight: $provider authorization URL passed"
done
...
if [[ -n "$AUTH_ADMIN_TOKEN" ]]; then
  ...
  echo "auth-stack live OAuth preflight: storage evidence passed"
fi

echo "auth-stack live OAuth preflight: passed"
