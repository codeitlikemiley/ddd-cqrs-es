...
- Projection catch-up from durable events can be expensive. `load_global_after_limited(...)` is the required store primitive and the only global read a runner performs; `run(...)` loops `run_batch(...)` until `caught_up`. Production examples should still drive catch-up with explicit `run_batch(...)` / `ProjectionBatchConfig` so one poll's duration is bounded, and reserve `load_global_after(...)` — which returns the whole backlog in one allocation — for small tests or explicit maintenance jobs.
