...
async function fetchStorageStatus() {
  if (!adminToken) {
    throw new Error("AUTH_ADMIN_TOKEN is required for live OAuth browser storage evidence.");
  }
...
async function runProvider(browser, provider) {
  ...
  try {
    console.log(`auth-stack live OAuth browser: starting ${provider}`);
    ...
    console.log(
      `auth-stack live OAuth browser: complete ${provider} login in the opened browser; waiting for /dashboard`,
    );
    await page.waitForURL(
      (currentUrl) => {
        const current = new URL(currentUrl);
    ...
    if (!sessionCookie?.value) {
      throw new Error(`${provider}: browser did not receive ddd_auth_session cookie`);
    }
    const cookieHeader = `ddd_auth_session=${sessionCookie.value}`;
    ...
    console.log(`auth-stack live OAuth browser: ${provider} passed`);
  } finally {
    await context.close();
  }
}
...
try {
  ...
  console.log(`auth-stack live OAuth browser: passed for ${providerList.join(" ")}`);
} finally {
  await browser.close();
}
