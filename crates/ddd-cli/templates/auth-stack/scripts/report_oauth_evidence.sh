...
if [[ -z "$AUTH_ADMIN_TOKEN" ]]; then
  echo "Error: AUTH_ADMIN_TOKEN is required for OAuth evidence." >&2
  exit 1
fi
...
echo "auth-stack OAuth evidence report"
...
if (( failures > 0 )); then
  echo "auth-stack OAuth evidence report: failed with $failures issue(s)" >&2
  exit 1
fi

echo "auth-stack OAuth evidence report: passed"
