...
session_cookie_header() {
  if [[ -n "$SESSION_COOKIE" ]]; then
    case "$SESSION_COOKIE" in
      Cookie:*) printf '%s\n' "$SESSION_COOKIE" ;;
      *ddd_auth_session=*) printf 'Cookie: %s\n' "$SESSION_COOKIE" ;;
      *) printf 'Cookie: ddd_auth_session=%s\n' "$SESSION_COOKIE" ;;
    esac
    return 0
  fi
...
if [[ -z "$AUTH_ADMIN_TOKEN" ]]; then
  echo "Error: AUTH_ADMIN_TOKEN is required for OAuth callback storage evidence." >&2
  exit 1
fi
...
echo "auth-stack live OAuth callback: checking authenticated session at $BASE_URL"
...
echo "auth-stack live OAuth callback: session and storage evidence passed for $OAUTH_PROVIDERS"
