use ddd_auth::{
    decode_id_token, encode_jwt, jwks_key_by_id, jwt_key_id, Algorithm, DecodingKey, EncodingKey,
    Header, IdTokenClaims,
    JwksDocument, JwksKey,
};
...
pub async fn complete_authorization_code(
    ...
    let config = oauth_provider_runtime_config(provider_id).await?;
    let token_response = exchange_authorization_code(&config, code).await?;
        ...
        .filter(|value| !value.trim().is_empty())
    {
        ...
        let claims = validate_provider_id_token(&config, id_token, &jwks, &grant.state)?;
...
async fn exchange_authorization_code(
    ...
    serde_json::from_slice(&response)
        .map_err(|error| AuthStackError::transport(format!("OAuth token response is invalid JSON: {error}")))
}

async fn provider_jwks(config: &OAuthProviderRuntimeConfig) -> AuthStackResult<JwksDocument> {
    if let Some(value) = config.jwks_json.as_deref().filter(|value| !value.trim().is_empty()) {
        return parse_jwks_document(value);
    }
    let Some(url) = config.jwks_url.as_deref().filter(|value| !value.trim().is_empty()) else {
    ...
    serde_json::from_slice(&response)
        .map_err(|error| AuthStackError::configuration(format!("OAuth JWKS response is invalid JSON: {error}")))
}
...
fn form_urlencoded(parts: &[(&str, &str)]) -> String {
    parts
        .iter()
        .map(|(key, value)| format!("{}={}", url_query_component(key), url_query_component(value)))
        .collect::<Vec<_>>()
        .join("&")
}
...
mod tests {
    ...
    fn id_token_algorithm_rejects_unsupported_key_type() {
        ...
        assert_eq!(id_token_algorithm(&key).unwrap_err().public_code(), "invalid_token");
    }
